...
from src.storage import SQLITE_BASE, StockDaily, NewsIntel, AnalysisHistory, BacktestResult, BacktestSummary


def generate_erd_markdown():
    """生成ERD图的Markdown表示"""
    
    md_content = f"""# 数据库ERD图 (实体关系图)
...
def main():
    ...
    output_file = output_dir / 'database_erd.md'
    
    md_content = generate_erd_markdown()
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(md_content)
    
    print(f"ERD图已生成: {output_file}")
    print("\n表结构概览:")
    for table_name, table in SQLITE_BASE.metadata.tables.items():
        print(f"\n  {table_name}:")
        for column in table.columns:
            pk_marker = " [PK]" if column.primary_key else ""
            print(f"    - {column.name}: {column.type}{pk_marker}")
